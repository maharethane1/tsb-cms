...
$performName = "Alt Kategori Yönetimi";
...
if (isset($_POST['add'])) {
    ...
    if ($_FILES['resim']['size']>0) {
        ...
        $benzersizad = $resimadi . "-urun-alt-kategori-" . $benzersizsayi1 . $benzersizsayi2 . $benzersizsayi3 . "." . $uzanti;
        $benzersizad2 = "t1-" . $resimadi . "-urun-alt-kategori-" . $benzersizsayi1 . $benzersizsayi2 . $benzersizsayi3 . "." . $uzanti;
        $benzersizad3 = "t2-" . $resimadi . "-urun-alt-kategori-" . $benzersizsayi1 . $benzersizsayi2 . $benzersizsayi3 . "." . $uzanti;
    ...
    try
    {
        $savePage = $db->prepare("INSERT INTO tbl_alt_kategori SET
        ...
        if ($savePage) {
            ...
            if ($saveLog) {
                Header("Location: ../../views/alt-kategori?statu=ok");
            } else {
                Header("Location: ../../views/alt-kategori?statu=logNo");
            }
        } else {

            Header("Location: ../../views/alt-kategori?statu=no");
        }
...
if (isset($_POST['edit'])) {
    ...
    {
        #Database kaydı
        $save = $db->prepare("UPDATE tbl_alt_kategori SET
            kategori_id=:kategori_id,
...
if ($_GET["perform"] == "delete") {
    ...
    if ($checkProduct>0) {
        return Header("Location: ../../views/alt-kategori?statu=hasComp");
    }

    $delete = $db->exec("DELETE from tbl_alt_kategori where id = $contentID");
...
if (isset($_POST['sort'])) {
    ...
    foreach ($items as $sira => $id) {
        ...
        #Database kaydı
        $save = $db->prepare("UPDATE tbl_alt_kategori SET sira=:sira WHERE id=$id");
        $save->execute(array(
            'sira' => $sira
        ));
    }
