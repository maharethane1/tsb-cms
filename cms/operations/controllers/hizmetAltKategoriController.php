...
$performName = "Hizmetler Alt Kategori Yönetimi";
...
if (isset($_POST['add'])) {
    ...
    if ($_FILES['resim']['size']>0) {
        ...
        $benzersizad = $resimadi . "-hizmet-altkategori-" . $benzersizsayi1 . $benzersizsayi2 . $benzersizsayi3 . "." . $uzanti;
        $benzersizad2 = "t1-" . $resimadi . "-hizmet-altkategori-" . $benzersizsayi1 . $benzersizsayi2 . $benzersizsayi3 . "." . $uzanti;
        $benzersizad3 = "t2-" . $resimadi . "-hizmet-altkategori-" . $benzersizsayi1 . $benzersizsayi2 . $benzersizsayi3 . "." . $uzanti;
    ...
    try
    {
        $savePage = $db->prepare("INSERT INTO tbl_hizmet_alt_kategori SET
        ...
        if ($savePage) {
            ...
            $saveLog->execute(array(
                'islem_id' => $contentID,
                'sayfa' => $performName,
                'islem' => 'Yeni alt kategori ekleme',
                'adi' => $userName,
                'icerik' => $adi,
                'uye_id' => $userID
            ));
            ...
            if ($saveLog) {
                Header("Location: ../../views/hizmet-alt-kategori?statu=ok");
            } else {
                Header("Location: ../../views/hizmet-alt-kategori?statu=logNo");
            }
        } else {

            Header("Location: ../../views/hizmet-alt-kategori?statu=no");
        }
...
if (isset($_POST['edit'])) {
    ...
    {
        #Database kaydı
        $save = $db->prepare("UPDATE tbl_hizmet_alt_kategori SET
            kategori_id=:kategori_id,
...
if ($_GET["perform"] == "delete") {
    ...
    $checkService = $db->query("SELECT COUNT(*) FROM tbl_hizmetler WHERE alt_kat_id=$contentID");
    $countService = $checkService->fetchColumn();

    if ($countService>0) {
        return Header("Location: ../../views/hizmet-alt-kategori?statu=hasService");
    }

    $delete = $db->exec("DELETE from tbl_hizmet_alt_kategori where id = $contentID");
...
if (isset($_POST['sort'])) {
    ...
    foreach ($items as $sira => $id) {
        ...
        #Database kaydı
        $save = $db->prepare("UPDATE tbl_hizmet_alt_kategori SET sira=:sira WHERE id=$id");
        $save->execute(array(
            'sira' => $sira
        ));
    }
